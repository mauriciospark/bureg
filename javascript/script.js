...
async function loadFromDataJson() {
  try {
    ...
    if (response.ok) {
      ...
      // Load prices from data.json
      if (productData.produtos) {
        const produtos = productData.produtos;

        // Load banana subtypes
        if (produtos.banana && produtos.banana.subtipos) {
          const subtipos = produtos.banana.subtipos;
          priceConfig.banana.pinguelo.compra =
            subtipos.pinguelo.precoCompra || 0;
          priceConfig.banana.pinguelo.venda = subtipos.pinguelo.precoVenda || 0;
          priceConfig.banana.filer.compra = subtipos.filer.precoCompra || 0;
          priceConfig.banana.filer.venda = subtipos.filer.precoVenda || 0;
          priceConfig.banana.ferrao.compra = subtipos.ferrao.precoCompra || 0;
          priceConfig.banana.ferrao.venda = subtipos.ferrao.precoVenda || 0;
          priceConfig.banana.media.compra = subtipos.media.precoCompra || 0;
          priceConfig.banana.media.venda = subtipos.media.precoVenda || 0;
        }

        // Load other products
        priceConfig.farinhaBanana.compra =
          produtos.farinhaBanana?.precoCompra || 0;
        priceConfig.farinhaBanana.venda =
          produtos.farinhaBanana?.precoVenda || 0;
        priceConfig.sacoAcai.compra = produtos.acai?.precoCompra || 0;
        priceConfig.sacoAcai.venda = produtos.acai?.precoVenda || 0;
        priceConfig.sacoCara.compra = produtos.cara?.precoCompra || 0;
        priceConfig.sacoCara.venda = produtos.cara?.precoVenda || 0;
        priceConfig.sacoFarinha.compra = produtos.farinha?.precoCompra || 0;
        priceConfig.sacoFarinha.venda = produtos.farinha?.precoVenda || 0;
      }

      console.log("Dados de produtos carregados do data.json:", productData);
      return true;
    }
  } catch (error) {
    console.error("Erro ao carregar JSON data:", error);
    return false;
  }
}
    ...
    if (response.ok) {
      ...
      if (memoryData.configuracao.precos) {
        ...
        priceConfig.farinhaBanana =
          precos.farinhaBanana || priceConfig.farinhaBanana;
    ...
    if (!memoryData) {
      memoryData = {
        meta: {
          ...
          nome: "Burég",
          descricao:
            "Sistema de gerenciamento de transporte fluvial para produtos agrícolas",
        ...
        dados: {
          ...
          estatisticas: {
            ...
            totalInvestimento: transportes.reduce(
              (sum, t) => sum + t.investimento,
              0,
            ),
            ...
            ultimoRegistro:
              transportes.length > 0
                ? transportes[transportes.length - 1].dataCadastro
                : null,
          },
        },
        historico: {
  ...
  try {
    ...
    if (backupData) {
      ...
      if (
        parsed.data &&
        confirm(
          `Deseja restaurar o backup de ${backupDate}? Esta ação não pode ser desfeita.`,
        )
      ) {
        ...
        responsavelPersonalizado =
          memoryData.meta.responsavelPersonalizado || "";
...
function getPrecoByProduct(produto, subtype = null) {
  if (produto === "banana" && subtype && priceConfig.banana[subtype]) {
    return {
      compra: priceConfig.banana[subtype].compra,
      venda: priceConfig.banana[subtype].venda,
    };
  }
  if (produto === "farinhaBanana")
    return {
      compra: priceConfig.farinhaBanana.compra,
      venda: priceConfig.farinhaBanana.venda,
    };
  if (produto === "acai")
    return {
      compra: priceConfig.sacoAcai.compra,
      venda: priceConfig.sacoAcai.venda,
    };
  if (produto === "cara")
    return {
      compra: priceConfig.sacoCara.compra,
      venda: priceConfig.sacoCara.venda,
    };
  if (produto === "farinha")
    return {
      compra: priceConfig.sacoFarinha.compra,
      venda: priceConfig.sacoFarinha.venda,
    };
...
function updatePrecoFields(produto, subtype = null) {
  ...
  document.getElementById("precoCompra").value = compra > 0 ? compra : "";
  document.getElementById("precoVenda").value = venda > 0 ? venda : "";

  // Atualizar o preview financeiro se houver valores
  if (compra > 0 || venda > 0) {
...
document.getElementById("configForm").addEventListener("submit", function (e) {
  ...
  responsavelPersonalizado = document
    .getElementById("configResponsavel")
    .value.trim();

  priceConfig.banana.pinguelo = {
    compra:
      parseFloat(document.getElementById("configPingueloCompra").value) || 0,
    venda:
      parseFloat(document.getElementById("configPingueloVenda").value) || 0,
  };
  ...
  priceConfig.banana.ferrao = {
    compra:
      parseFloat(document.getElementById("configFerraoCompra").value) || 0,
  ...
  priceConfig.farinhaBanana = {
    compra:
      parseFloat(document.getElementById("configFarinhaBananaCompra").value) ||
      0,
    venda:
      parseFloat(document.getElementById("configFarinhaBananaVenda").value) ||
      0,
  };
  priceConfig.sacoAcai = {
    compra:
      parseFloat(document.getElementById("configSacoAciaCompra").value) || 0,
    venda:
      parseFloat(document.getElementById("configSacoAciaVenda").value) || 0,
  };
  priceConfig.sacoCara = {
    compra:
      parseFloat(document.getElementById("configSacoCaraCompra").value) || 0,
    venda:
      parseFloat(document.getElementById("configSacoCaraVenda").value) || 0,
  };
  priceConfig.sacoFarinha = {
    compra:
      parseFloat(document.getElementById("configSacoFarinhaCompra").value) || 0,
    venda:
      parseFloat(document.getElementById("configSacoFarinhaVenda").value) || 0,
  };
...
document.getElementById("tipoOperacao").addEventListener("change", function () {
  ...
  const precoCompraGroup = document.getElementById("precoCompraGroup");
  const precoVendaGroup = document.getElementById("precoVendaGroup");
  ...
  if (tipoOperacao === "compra") {
    precoCompraGroup.classList.remove("hidden");
    precoVendaGroup.classList.add("hidden");
    precoVendaInput.required = false;
    precoCompraInput.required = true;
  } else if (tipoOperacao === "venda") {
    precoCompraGroup.classList.add("hidden");
    precoVendaGroup.classList.remove("hidden");
    precoCompraInput.required = false;
    precoVendaInput.required = true;
  } else {
    precoCompraGroup.classList.remove("hidden");
    precoVendaGroup.classList.remove("hidden");
    precoCompraInput.required = true;
...
// Carregar preços iniciais se um produto já estiver selecionado (caso de recarregamento de página)
document.addEventListener("DOMContentLoaded", function () {
  const produtoSelecionado = document.getElementById("produto").value;
  if (produtoSelecionado && productRules[produtoSelecionado]) {
    const subtipoSelecionado = document.getElementById("bananaSubtype").value;
    updatePrecoFields(produtoSelecionado, subtipoSelecionado);
  }
});

document
  .getElementById("bananaSubtype")
...
function updateFinancialPreview() {
  ...
  const precoCompra =
    parseFloat(document.getElementById("precoCompra").value) || 0;
  const precoVenda =
    parseFloat(document.getElementById("precoVenda").value) || 0;
  ...
  if (tipoOperacao === "compra" && quantidade > 0 && precoCompra > 0) {
    investimento = quantidade * precoCompra;
    lucro = -investimento;
    showPreview = true;
  } else if (tipoOperacao === "venda" && quantidade > 0 && precoVenda > 0) {
    retorno = quantidade * precoVenda;
    lucro = retorno;
    showPreview = true;
  } else if (
    !tipoOperacao &&
  ...
  if (showPreview) {
    ...
    fields.forEach(
      ([id, value]) =>
        (document.getElementById(id).textContent = `R$ ${value.toFixed(2)}`),
    );
    document.getElementById("previewLucro").className =
...
document
  .getElementById("tipoOperacao")
  .addEventListener("change", debounce(updateFinancialPreview, 150));
document
  .getElementById("quantidade")
  .addEventListener("input", debounce(updateFinancialPreview, 150));
document
...
document.getElementById("clearBtn").addEventListener("click", function () {
  ...
  document.getElementById("productInfo").classList.add("hidden");
  document.getElementById("financialPreview").classList.add("hidden");
  document.getElementById("precoCompraGroup").classList.remove("hidden");
  document.getElementById("precoVendaGroup").classList.remove("hidden");
  document.getElementById("precoCompra").required = true;
  document.getElementById("precoVenda").required = true;
});
...
function validateForm(formData) {
  ...
  if (!formData.tipoOperacao) {
    errors.push("Selecione o tipo de operação");
  }
  ...
  if (
    ...
    formData.precoVenda < formData.precoCompra * 1.1
  ) {
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    // Calcular valores baseados no tipo de operação
    let investimento = 0;
    let retorno = 0;
    let lucro = 0;

    if (formData.tipoOperacao === "compra") {
      investimento = formData.quantidade * formData.precoCompra;
      retorno = 0; // Na compra, não há retorno ainda
      lucro = -investimento; // Compra é considerada investimento negativo
    } else if (formData.tipoOperacao === "venda") {
      investimento = 0; // Na venda, não há investimento
      retorno = formData.quantidade * formData.precoVenda;
      lucro = retorno; // Venda é considerada lucro direto
    } else {
      // Modo compatibilidade (ambos os preços)
      investimento = formData.quantidade * formData.precoCompra;
      retorno = formData.quantidade * formData.precoVenda;
      lucro = retorno - investimento;
    }
...
document
  .getElementById("filterOperation")
  .addEventListener("change", function () {
    currentOperationFilter = this.value;
    renderTransportes();
  });

document.getElementById("searchTransport").addEventListener(
  "input",
  debounce(function () {
...
function renderTransportes() {
  ...
  let filtered = transportes.filter((t) => {
    ...
    if (
      currentSearch &&
      !t.nomeProduto.toLowerCase().includes(currentSearch) &&
      !t.subtipo?.toLowerCase().includes(currentSearch)
    )
      return false;
    ...
    if (currentDateFilter !== "all") {
      ...
      switch (currentDateFilter) {
        case "today":
          if (transportDate.toDateString() !== today.toDateString())
            return false;
          break;
        case "week":
        ...
        case "month":
          if (
            transportDate.getMonth() !== today.getMonth() ||
            transportDate.getFullYear() !== today.getFullYear()
          )
            return false;
          break;
        case "year":
    ...
    if (currentOperationFilter !== "all") {
      if (currentOperationFilter === "compra" && t.tipoOperacao !== "compra")
        return false;
      if (currentOperationFilter === "venda" && t.tipoOperacao !== "venda")
        return false;
    }
  ...
  if (filtered.length === 0) {
    transportList.innerHTML =
      '<p class="empty-message">Nenhuma carga encontrada com os filtros atuais.</p>';
    return;
  }
  ...
  filtered.forEach((transporte) => {
    ...
    // Mostrar tipo de operação
    const operationEl = clone.querySelector(".compact-operation");
    if (transporte.tipoOperacao) {
      const operationBadge =
        transporte.tipoOperacao === "compra" ? "💰 COMPRA" : "🏪 VENDA";
      operationEl.textContent = operationBadge;
      operationEl.style.display = "inline";
      operationEl.style.marginLeft = "8px";
      operationEl.style.fontSize = "0.8em";
      operationEl.style.padding = "2px 6px";
      operationEl.style.borderRadius = "4px";
      operationEl.style.background =
        transporte.tipoOperacao === "compra" ? "#fed7d7" : "#c6f6d5";
      operationEl.style.color =
        transporte.tipoOperacao === "compra" ? "#c53030" : "#2f855a";
    } else {
      operationEl.style.display = "none";
    }

    clone.querySelector(".compact-date").textContent = new Date(
      transporte.dataCadastro,
    ...
    const lucroEl = clone.querySelector(
      ".financial-item.profit span:last-child",
    );
...
function updateSummary() {
  ...
  const totalInvestimento = transportes.reduce(
    (sum, t) => sum + t.investimento,
    0,
  );
  ...
  document.getElementById("totalRetorno").textContent =
    `R$ ${totalRetorno.toFixed(2)}`;
  document.getElementById("totalLucro").textContent =
    `R$ ${totalLucro.toFixed(2)}`;
  document.getElementById("totalLucro").className =
    totalLucro >= 0 ? "positive" : "negative";
}
...
document.getElementById("editForm").addEventListener("submit", function (e) {
  ...
  const precoCompra = parseFloat(
    document.getElementById("editPrecoCompra").value,
  );
  const precoVenda = parseFloat(
    document.getElementById("editPrecoVenda").value,
  );

  transportes[index].quantidade = quantidade;
  transportes[index].paradas = parseFloat(
    document.getElementById("editParadas").value,
  );
  ...
  transportes[index].lucro =
    transportes[index].retorno - transportes[index].investimento;
  transportes[index].observacoes =
...
const memoizedRenderTransportes = memoize(
  (filter, sort, search, dateFilter, profitFilter) => {
...
function openSalesReceiptForm(transporteId) {
  console.log(
    "Abrindo formulário de comprovante para transporte ID:",
    transporteId,
  );
...
function generateStatistics() {
  ...
  clone.querySelector("#statTotalInvestimento").textContent =
    `R$ ${totalInvestimento.toFixed(2)}`;
  clone.querySelector("#statTotalRetorno").textContent =
    `R$ ${totalRetorno.toFixed(2)}`;

  const lucroElement = clone.querySelector("#statTotalLucro");
  lucroElement.textContent = `R$ ${totalLucro.toFixed(2)}`;
  lucroElement.className =
    totalLucro >= 0 ? "stat-value positive" : "stat-value negative";

  // Product distribution chart
  const productChart = clone.querySelector("#productChart");
  const maxQuantity = Math.max(
    ...Object.values(productStats).map((p) => p.quantidade),
  );
  ...
  const maxProfit = Math.max(
    ...Object.values(productStats).map((p) => Math.abs(p.lucro)),
  );
  ...
  Object.entries(productStats).forEach(([key, stats]) => {
    ...
    const margem =
      stats.investimento > 0 ? (stats.lucro / stats.investimento) * 100 : 0;
...
function generateSalesReceipt(transporteId = null, customerData = null) {
  ...
  const discount = customerData?.discount
    ? parseFloat(customerData.discount)
    : 0;
  ...
  clone.querySelector(".receipt-customer-name").textContent =
    customerData?.name || "Não informado";
  clone.querySelector(".receipt-customer-doc").textContent =
    customerData?.doc || "Não informado";
  clone.querySelector(".receipt-customer-phone").textContent =
    customerData?.phone || "Não informado";

  clone.querySelector(".receipt-product-name").textContent =
    `${transporte.icon} ${transporte.nomeProduto}`;
  clone.querySelector(".receipt-quantity").textContent =
    `${transporte.quantidade} un`;
  clone.querySelector(".receipt-paradas").textContent = transporte.paradas;

  clone.querySelector(".receipt-preco-compra").textContent =
    `R$ ${transporte.precoCompra.toFixed(2)}`;
  clone.querySelector(".receipt-preco-venda").textContent =
    `R$ ${transporte.precoVenda.toFixed(2)}`;
  clone.querySelector(".receipt-discount").textContent =
    discount > 0 ? `- R$ ${discount.toFixed(2)}` : "R$ 0,00";
  clone.querySelector(".receipt-total-compra").textContent =
    `R$ ${transporte.investimento.toFixed(2)}`;
  clone.querySelector(".receipt-total-venda").textContent =
    `R$ ${totalComDesconto.toFixed(2)}`;
  clone.querySelector(".receipt-payment-method").textContent =
    paymentMethods[customerData?.paymentMethod] || "Não informado";

  const lucroElement = clone.querySelector(".receipt-lucro");
  lucroElement.textContent = `R$ ${lucroComDesconto.toFixed(2)}`;
  lucroElement.className =
    lucroComDesconto >= 0 ? "receipt-lucro positive" : "receipt-lucro negative";

  clone.querySelector(".receipt-observacoes").textContent =
    transporte.observacoes || "Sem observações";
...
function generateCertificate() {
  ...
  clone.querySelector(".certificate-header p:last-child span").textContent =
    nomeResponsavel;

  const infoItems = clone.querySelectorAll(
    ".certificate-info-item span:last-child",
  );
  ...
  const firstDate =
    transportes.length > 0
      ? new Date(transportes[0].dataCadastro).toLocaleDateString("pt-BR")
      : "N/A";
  const lastDate =
    transportes.length > 0
      ? new Date(
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  document
    .getElementById("closeCertificateModal")
    .addEventListener("click", () =>
  ...
  document
    .getElementById("salesReceiptBtn")
    .addEventListener("click", function () {
      if (transportes.length === 0) {
        showMessage(
          "error",
          "Não há cargas cadastradas para gerar comprovante!",
        );
        return;
      }
  ...
  document
    .getElementById("closeSalesReceiptModal")
    .addEventListener("click", () =>
      document.getElementById("salesReceiptModal").classList.add("hidden"),
    );
  document
    .getElementById("closeSalesReceiptBtn")
    .addEventListener("click", () =>
      document.getElementById("salesReceiptModal").classList.add("hidden"),
    );
  ...
  document
    .getElementById("cancelReceiptBtn")
    .addEventListener("click", function () {
      document.getElementById("salesReceiptModal").classList.add("hidden");
    });
  ...
  document
    ...
    .addEventListener("click", function () {
      const receiptContent = document
        .getElementById("salesReceiptContent")
        .cloneNode(true);

      // Ensure logo is visible
      const logoImages = receiptContent.querySelectorAll(
      ...
      showMessage(
        "success",
        'Arquivo HTML exportado! Abra o arquivo e use "Salvar como PDF" no navegador.',
        6000,
      );
    });

  document
    .getElementById("printSalesReceipt")
    .addEventListener("click", function () {
      window.print();
    });
  ...
  document.getElementById("configBtn").addEventListener("click", function () {
    ...
    document.getElementById("configPingueloCompra").value =
      priceConfig.banana.pinguelo.compra || "";
    document.getElementById("configPingueloVenda").value =
      priceConfig.banana.pinguelo.venda || "";
    document.getElementById("configFilerCompra").value =
      priceConfig.banana.filer.compra || "";
    document.getElementById("configFilerVenda").value =
      priceConfig.banana.filer.venda || "";
    document.getElementById("configFerraoCompra").value =
      priceConfig.banana.ferrao.compra || "";
    document.getElementById("configFerraoVenda").value =
      priceConfig.banana.ferrao.venda || "";
    document.getElementById("configMediaCompra").value =
      priceConfig.banana.media.compra || "";
    document.getElementById("configMediaVenda").value =
      priceConfig.banana.media.venda || "";
    document.getElementById("configFarinhaBananaCompra").value =
      priceConfig.farinhaBanana.compra || "";
    document.getElementById("configFarinhaBananaVenda").value =
      priceConfig.farinhaBanana.venda || "";
    document.getElementById("configSacoAciaCompra").value =
      priceConfig.sacoAcai.compra || "";
    document.getElementById("configSacoAciaVenda").value =
      priceConfig.sacoAcai.venda || "";
    document.getElementById("configSacoCaraCompra").value =
      priceConfig.sacoCara.compra || "";
    document.getElementById("configSacoCaraVenda").value =
      priceConfig.sacoCara.venda || "";
    document.getElementById("configSacoFarinhaCompra").value =
      priceConfig.sacoFarinha.compra || "";
    document.getElementById("configSacoFarinhaVenda").value =
      priceConfig.sacoFarinha.venda || "";
